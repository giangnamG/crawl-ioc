    ...
    def keywords():
        with connect() as conn:
            if request.method == "POST":
                ...
                for text in parse_lines(raw_keywords):
                    ...
                    for dork in dorks:
                        ...
                        search_query_id, created = upsert_search_query(
                            conn, keyword_id, dork["id"], query_text
                        )
                        if created:
                            queries_created += 1
                        conn.execute(
                        ...
                        enqueue_job(
                            ...
                            f"search:{search_query_id}",
                        )

                flash(
                    f"Imported {keywords_imported} lines and queued {queries_created} new search queries.",
                    "success",
                )
        ...
        return render_template("keywords.html", dorks=dorks, rows=rows)
...
def resume_keyword_work(conn, keyword_id: int) -> int:
    query_rows = conn.execute(
        ...
        SELECT id
        FROM search_queries
        WHERE keyword_id = ? AND status = 'paused'
        ORDER BY id
        """,
        (keyword_id,),
    ).fetchall()
    ...
    for query_id in query_ids:
        ...
        enqueue_job(conn, "search_query", {"search_query_id": query_id}, f"search:{query_id}")
...
def delete_keyword_from_queue(conn, keyword_id: int) -> tuple[bool, int, int, str]:
    ...
    if query_ids:
        ...
        job_keys = [f"search:{query_id}" for query_id in query_ids]
        job_placeholders = ",".join("?" for _ in job_keys)
        job_cursor = conn.execute(
            f"""
            DELETE FROM jobs
            WHERE type = 'search_query'
              AND dedupe_key IN ({job_placeholders})
            """,
            job_keys,
        )
...
def resume_paused_jobs(conn) -> tuple[int, int]:
    paused_search_jobs = conn.execute(
        "SELECT * FROM jobs WHERE status = 'paused' AND type = 'search_query'"
    ).fetchall()
    search_query_ids = search_query_ids_from_jobs(paused_search_jobs)
    resumed_jobs = conn.execute(
        """
        UPDATE jobs
        SET status = 'pending',
            started_at = NULL,
    ...
    for query_id in search_query_ids:
        ...
        if row:
            affected_keyword_ids.add(int(row["keyword_id"]))
        conn.execute("UPDATE search_queries SET status = 'pending' WHERE id = ?", (query_id,))

    for keyword_id in affected_keyword_ids:
        conn.execute("UPDATE keywords SET status = 'pending' WHERE id = ?", (keyword_id,))
        refresh_keyword_status(conn, keyword_id)

    return max(resumed_jobs, 0), len(search_query_ids)
...
def upsert_search_query(conn, keyword_id: int, dork_id: int, query_text: str) -> tuple[int, bool]:
    before = conn.total_changes
    conn.execute(
        """
        INSERT OR IGNORE INTO search_queries(keyword_id, dork_id, query_text)
        VALUES (?, ?, ?)
        """,
    ...
    row = conn.execute(
        "SELECT id FROM search_queries WHERE keyword_id = ? AND dork_id = ? AND query_text = ?",
        (keyword_id, dork_id, query_text),
    ).fetchone()
